...
{{"claim": "<the exact claim, self-contained and readable out of context>"}}
...
def extract_claims(client: genai.Client, doc_text: str, max_claims: int = MAX_CLAIMS) -> list[dict]:
...
def verify_claim(client: genai.Client, claim: str, evidence: list[dict]) -> dict:
    evidence_text = "\n".join(
...
with st.sidebar:
    ...
    if has_secret:
        ...
        st.session_state["api_key_input"] = st.text_input(
            "Gemini API key", type="password", help="Get one free at https://aistudio.google.com/apikey"
        )
    st.markdown("---")
    st.caption("Free web search via DuckDuckGo — no extra API key needed.")
    st.caption(f"Model: `{MODEL_NAME}` · Max claims per run: {MAX_CLAIMS}")
...
if run:
    api_key = get_api_key()
    if not api_key:
        st.error("Please add a Gemini API key in the sidebar (or as a deployed secret) before running.")
        st.stop()
